...
extension [R, E, T](t: ZIO[R, E, Seq[T]])
  ...
  def atMostOne(using DbEntityName[T]): ZIO[R, E | DbLookupException.MultipleResults[T], Option[T]] = t.flatMap:
    chunk => ZIO.cond(chunk.size < 2, chunk.headOption, DbLookupException.MultipleResults[T])
