package typebricks.scalasql.dbzio
...
final class DbOpLocation(using
    val file: FileName,
    val line: Line,
    val trace: Trace
)
...
object DbOp:
  ...
  /** Execute a SELECT expecting exactly one result. Fails with `DbLookupException` on 0 or 2+ results. */
  def runSingle[Q, R](query: Select[Q, R])(using DbOpLocation, DbEntityName[R]): DbIO[DbLookupException[R], R] =
...
extension [UnsafeApi](useBlock: UseBlock[UnsafeApi])
  private def zScope[SafeApi](wrap: UnsafeApi => SafeApi)(using DbOpLocation): ZIO[Scope, DbException, SafeApi] =
    val acquire = DbRun:
  ...
  private def zLift[SafeApi, R, E, A](wrap: UnsafeApi => SafeApi)(safeUse: SafeApi => ZIO[R, E, A])(using
      DbOpLocation
  ): ZIO[R, DbException | E, A] =
    ZIO.scoped:
      zScope(wrap).flatMap(safeUse)

/** ZIO wrapper around scalasql's `DbApi`. Execute queries as ZIO effects with automatic error wrapping. */
...
object ZDbApi:
  def autoCommitDataSource(ds: DataSource)(using DialectConfig, Trace)(using FileName, Line): DbRun[ZDbApi] = DbRun:
...
class ZDbTxnApi(unsafe: UnsafeDbApi.Txn) extends ZDbApi(unsafe):
  ...
  /** Execute a block within a savepoint. The savepoint is released on success and rolled back on failure. */
  def savepoint[R, E, A](use: Savepoint => ZIO[R, E, A])(using location: DbOpLocation): ZIO[R, DbException | E, A] =
...
class ZDbClient(unsafe: UnsafeDbClient):
  ...
  def withAutoCommits[R, E, A](use: ZIO[ZDbApi & R, E, A])(using DbOpLocation): ZIO[R, DbException | E, A] =
  ...
  def transaction[R, E, A](use: ZIO[ZDbTxnApi & R, E, A])(using DbOpLocation): ZIO[R, DbException | E, A] =
    unsafe.transaction.zLift(new ZDbTxnApi(_))(client => use.provideSomeEnvironment[R](_.add(client)))

  /** Acquire a scoped transaction. Commits when the scope closes successfully, rolls back on failure. */
...
object ZDbClient extends AccessCompanion[ZDbClient]:
  ...
  def dataSource(ds: DataSource, config: scalasql.Config)(using DialectConfig): ZDbClient = new ZDbClient(
    UnsafeDbClient.DataSource(ds, config)
  )

  /** Run a ZIO effect within a transaction, pulling the `ZDbClient` from the environment. */
  def transaction[R, E, A](use: ZIO[ZDbTxnApi & R, E, A])(using DbOpLocation): ZIO[ZDbClient & R, DbException | E, A] =
    apply(_.transaction(use))

  /** Run a ZIO effect with auto-committing database access, pulling the `ZDbClient` from the environment. */
  def withAutoCommits[R, E, A](use: ZIO[ZDbApi & R, E, A])(using DbOpLocation): ZIO[ZDbClient & R, DbException | E, A] =
