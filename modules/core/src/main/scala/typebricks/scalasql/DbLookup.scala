...
object DbLookupException:
  final class MultipleResults[T: DbEntityName as name] extends DbLookupException(s"Multiple entries of $name exist")

  final class NotFound[T: DbEntityName as name] extends DbLookupException(s"Expected $name does not exist")
...
object DbEntityName:
  ...
  def typeReprImpl[T: Type](using q: Quotes): Expr[String] = Expr(
    q.reflect.TypeRepr.of[T].dealias.simplified.show.split('.').filterNot(_.endsWith("$package")).mkString(".")
  )
